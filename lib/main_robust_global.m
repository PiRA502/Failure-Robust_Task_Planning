function [W, ZLoop, mytimes, sol, obj] = main_robust_global(formula,A,h,W0,Obs,CA_flag,k,gurobi_method,alpha, opt)
% failure-robust algorithm for global robustness 
...
for j = 1:N 
    for b = 1 : h 
        eval(['W_R',int2str(idx),' = binvar(repmat(I,1,N),repmat(h+1,1,N),''full'');']);
        for t = 1 : (h+1)  
            if (t >= b)    
                eval(['W_R',int2str(idx),'{1,j}(:,t) = zeros(I,1);']);  
            else
                eval(['fRobust = [fRobust, W_R',int2str(idx),'{1,j}(:,t) == W{1,j}(:,t)];']);
            end
        end
        idx = idx + 1;
    end         
...
for i = 0:(N-1)
    for t =  1:h
        for j = 1:N  
            if (j ~= (i+1))
            eval(['fRobust = [fRobust, W_R',int2str(i*h + t),'{j} == W{j}];']);
            end
        end
    end
end
...
% All Constraints
F = [fInit, fDyn, fLoop, fObs, fRobust];
for i = 1:k_num
...
if opt == true
    ...
    sol = optimize(F,{},options); %第一个是约束，第二个是优化目标（min），第三个是option
end
tme=toc(tms); 
